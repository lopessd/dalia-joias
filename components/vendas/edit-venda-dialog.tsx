...
export function EditVendaDialog({ open, onOpenChange, venda }: EditVendaDialogProps) {
  ...
  useEffect(() => {
    if (open && venda) {
      setProdutosSelecionados([...(venda.products || [])])
      setObservacoes(venda.notes || "")
    }
  }, [open, venda])
  ...
  const adicionarProduto = (produto: any) => {
    setProdutosSelecionados([
      ...produtosSelecionados,
      {
        ...
        total_price: produto.precoVenda,
      },
    ])
    setSearchTerm("")
  }
  ...
  const atualizarQuantidade = (id: string, quantidade: number) => {
    ...
    setProdutosSelecionados(
      produtosSelecionados.map((produto) => (produto.id === id ? { ...produto, quantity: quantidade, total_price: quantidade * produto.unit_price } : produto)),
    )
  }
  ...
  const atualizarPreco = (id: string, preco: number) => {
    if (preco < 0) return
    setProdutosSelecionados(
      produtosSelecionados.map((produto) => (produto.id === id ? { ...produto, unit_price: preco, total_price: produto.quantity * preco } : produto)),
    )
  }

  const calcularTotal = () => {
    return produtosSelecionados.reduce((total, produto) => total + produto.quantity * produto.unit_price, 0)
  }

  const calcularQuantidadeTotal = () => {
    return produtosSelecionados.reduce((total, produto) => total + produto.quantity, 0)
  }
                      ...
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-sm font-body text-foreground">{produto.name}</span>
                          </div>
                ...
                <div className="space-y-4">
                  {produtosSelecionados.map((produto) => (
                    <div key={produto.id} className="flex items-center gap-4 p-4 bg-muted rounded-lg">
                      <div className="flex-1">
                        <p className="font-body text-foreground">{produto.name}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => atualizarQuantidade(produto.id, produto.quantity - 1)}
                        >
                          <Minus className="w-3 h-3" />
                        </Button>
                        <span className="w-8 text-center font-body">{produto.quantity}</span>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => atualizarQuantidade(produto.id, produto.quantity + 1)}
